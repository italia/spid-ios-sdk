...
class SPIDService {
    ...
    func getIdentityProviders(completionHandler: @escaping (DataResponse<ProvidersResponse>) -> Void, errorHandler: @escaping () -> Void) {
        ...
        let URL = "http://www.mocky.io/v2/59d8e6020f00006d122a2483" //String(format:"%@%@", baseUrl, ServicesUrlConstants.ELENCO_PROVIDERS_PATH)
    ...
    func getAuthenticationSpid(entityId: String, completionHandler: @escaping (DataResponse<AuthSpeedResponse>) -> Void, errorHandler: @escaping () -> Void) {
        let URL = "http://www.mocky.io/v2/59d95ff0100000a606caf130"//String(format:"%@%@?entityId=%@", baseUrl, ServicesUrlConstants.AUTH_SPID_PATH, entityId)
        Alamofire
            .request(URL)
